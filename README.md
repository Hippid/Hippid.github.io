...
## Folder Structure of most important Files.

```
my-app/
  README.md
  node_modules/             Git Ignored, contains NPM packages specified in package.json
  package.json
  public/
    index.html
    favicon.ico
  src/
    components/
        ...
        Comments/           
            CommentBox.Js
        Jobs/               Job Scraping component build with new CSS Grid, build with upcoming Subgrid feature in mind. 
            JobBox.js       Unfortunately subgrid is not supported yet by all mayor browsers and has been commented out.
        Menu/               Simple Button Menu to toggle between examples.
            MenuBox.js
        MenuBs/             Bootstrap 4 Menu bar with custom made Theme Switching.
            MenuBsBox.js    
        TvShows/            Simple Redux example that shows state preservation after external api call to TvMaze.     
            TvShowBox.js    Try switching between examples after retieving the Tv data to see the preserved the state.
    index.css
    index.js                Encapsulates App with redux provider. App can also be used without Provider when TvShows example isn't used.
    logo.svg
  actions/                  Redux actions for TvShows example
  epics/                    Redux epics for TvShows example
  reducers/                 Redux reducers for TvShows example
```
